...
Suppose we have a dataset \\( \{(x^1,y^1),...,(x^n,y^n)\} \\), 
\\( \\{(x^1,y^1),...,(x^n,y^n)\\} \\)
where \\(x^i\in\mathbb{R}^d\\) and \\(y^i\in\mathbb{R}\\). We want to find a linear function \\(f(x)=w^Tx+b\\) to fit the data. 
...
\right\|^2
$$
Then, X^T \\(\in \mathbb{R}^{n\times d}\\), Y \\(\in \mathbb{R}^{n}\\0, w \\(\in \mathbb{R}^{d}\\）. The problem is also \\(\underset{w}{\operatorname{argmin}} \frac{1}{2}\|Y-Xw\|^2_2\\).
